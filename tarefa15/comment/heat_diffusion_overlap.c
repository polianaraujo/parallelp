...
void exchange_and_compute(double *u, double *u_new, int local_n, int rank, int size, MPI_Comm comm) {
    MPI_Request reqs[4];
    int flag[2] = {0, 0};
    ...
    // Computar pontos internos enquanto espera a borda
    for (int i = 2; i <= local_n - 1; ++i)
        u_new[i] = u[i] + ALPHA * (u[i - 1] - 2 * u[i] + u[i + 1]);

    // Esperar chegada das bordas
...
int main(int argc, char **argv) {
    ...
    for (int step = 0; step < STEPS; ++step) {
        exchange_and_compute(u, u_new, local_n, rank, size, comm);
    ...
    if (rank == 0)
        printf("Tempo (MPI_Test com sobreposição): %.6f s\n", end - start);
