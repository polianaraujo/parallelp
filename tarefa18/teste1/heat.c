// # Versão modificada de heat.c com OpenMP GPUOffloading

/* Modificações principais:
 - Uso de #pragma omp target data para movimentar u e u_tmp apenas uma vez.
 - Inclusão de regiões target nos loops paralelizáveis.
*/
...
int main(int argc, char *argv[]) {
    ...
    // Envio dos dados à GPU e permanência durante os passos
    #pragma omp target data map(tofrom: u[0:n*n], u_tmp[0:n*n])
    {
    // Início do cronômetro da solução
    ...
    // Fim do cronômetro da solução
    double toc = omp_get_wtime();

    // Cálculo do erro no host
