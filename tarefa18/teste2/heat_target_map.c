...
int main(int argc, char *argv[]) {
  ...
  for (int t = 0; t < nsteps; ++t) {
    ...
    // Pointer swap (host-side swap works because associations between host and device addresses are fixed with the target data region)
    tmp = u;
    u = u_tmp;
    u_tmp = tmp;
  }
...
// Updated solve() routine to remove map clauses
void solve(const int n, const double alpha, const double dx, const double dt, const double * restrict u, double * restrict u_tmp) {
  ...
  #pragma omp target // No map clauses here, data is already on the device due to target data region
