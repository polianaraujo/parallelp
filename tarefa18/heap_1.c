...
** Modificado para OpenMP Offloading por Gemini, Maio 2024
*/
...
int main(int argc, char *argv[]) {
  ...
  // Esta função é executada na CPU, então os dados precisam estar no host
  ...
  // Entra na região de dados do target.
  // u é mapeado para o dispositivo e inicializado com os dados do host.
  // u_tmp é mapeado para o dispositivo (apenas alocado, não precisa de inicialização, pois será preenchido pela GPU).
  #pragma omp target enter data map(to: u[0:n*n]) map(alloc: u_tmp[0:n*n])
  for (int t = 0; t < nsteps; ++t) {

    // A chamada para solve ocorrerá na GPU.
    // Os ponteiros u e u_tmp já estão no dispositivo.
    // O OpenMP se encarrega de associar os ponteiros do host aos do dispositivo.
    // Não precisamos de omp target update aqui se os ponteiros são trocados no host
    // e a próxima iteração do loop target usar os ponteiros atualizados.
    // A maneira mais performática é ter a função solve trocando os ponteiros de forma implícita
    // ou passando-os de forma diferente, mas para o estilo do exercício, mantemos o swap no host.
    #pragma omp target
    solve(n, alpha, dx, dt, u, u_tmp);

    // Pointer swap
    // Este swap de ponteiros opera nos ponteiros do host.
    // O OpenMP 5.0+ gerencia melhor a associação automática dos ponteiros
    // após o swap se eles foram previamente mapeados com 'target data'.
    // Em versões anteriores ou para maior clareza, 'map(tofrom: ...)' na chamada da função solve
    // ou 'target update' seria necessário para cada iteração, o que é menos eficiente.
    tmp = u;
    u = u_tmp;
    u_tmp = tmp;
  }
  // Sai da região de dados do target, trazendo os dados finais de 'u' de volta para o host.
...
// Esta função é executada na CPU
void initial_value(const int n, const double dx, const double length, double * restrict u) {
...
void solve(const int n, const double alpha, const double dx, const double dt, const double * restrict u, double * restrict u_tmp) {
  ...
  // #pragma omp target indica que este código é executado no dispositivo.
  // #pragma omp teams distribute parallel for collapse(2) paralela os dois loops aninhados
  // para que as equipes de threads da GPU trabalhem em conjunto.
