...
void run_simulation_fraca(int num_threads, int nx, int ny, const char *schedule_type, int chunk_size) {
    ...
    FILE *file = fopen("results_escalabilidade_fraca.csv", "a");
    if (file) {
...
int main() {
    FILE *file = fopen("results_escalabilidade_fraca.csv", "w");
    ...
    for (int t = 0; t < 5; t++) {
        ...
        // Tamanho do problema proporcional ao número de threads
